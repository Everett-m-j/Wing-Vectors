...
# Helper to check if alignment caused a reflection (mirror flip)
is_reflected <- function(coordsL, coordsR) {
  # Center both shapes
  m1 <- coordsL - colMeans(coordsL)
  m2 <- coordsR - colMeans(coordsR)
  # SVD to get rotation matrix
  svd1 <- svd(t(m1) %*% m2)
  # If the determinant is negative, a reflection occurred
  det(svd1$u %*% t(svd1$v)) < 0
}
...
for (svg_L in svg_L_files) {
  ...
  # Now use `final_result` to write to your CSV:
  data_existing[idx_L, c("percent_asymmetry", "area", "circularity", "length", "width", "percent_overlap")] <-
    list(percent_asym, area1, circ1, dims1$length, dims1$width, best_overlap)
  
  data_existing[idx_R, c("percent_asymmetry", "area", "circularity", "length", "width", "percent_overlap")] <-
    list(percent_asym, area2, circ2, dims2$length, dims2$width, best_overlap)
}
...
plot(best_efi1, type = "l", col = "red", asp = 1)
lines(efi2, col = "blue")
# Only process "_L_4" files (each with a matching "_R_4")
svg_L <- ("/Users/everettmathiason/Documents/CCBER_Bee_Data/Data/Panurginus_polytrichus/Tracings_1st_Round/UCSB-IZC00058724_L_4.svg")
